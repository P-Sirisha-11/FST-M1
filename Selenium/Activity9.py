...
with webdriver.Firefox() as driver:
    ...
    for i in range(1,11):
        ...
        for name in row:
            name = driver.find_element(By.XPATH, f"{rows_xpath_text}/td[@field = 'name']//a")
            user = driver.find_element(By.XPATH, f"{rows_xpath_text}/td[@field = 'assigned_user_name']//a")
            print(f"Name of lead {i}: {name.text}")
            print(f"Assigned user name of lead {i}: {user.text}")
